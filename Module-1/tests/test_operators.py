...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert sigmoid(a) > 0.0 or sigmoid(a) < 1.0
    assert_close(1 - sigmoid(a), sigmoid(neg(a)))
    if a == 0.0:
        assert sigmoid(a) == 0.5
    assert sigmoid(a) * (1 - sigmoid(a)) >= 0.0
...
def test_symmetric(a: float, b: float) -> None:
    """
    Checks that the mutliplication of a * b = b * a
...
def test_other(a: float, b: float) -> None:
    """
    Ensures that:
        * the multiplicative identity holds (a*1 = a)
        * the zero identity for addition holds (a +0 =a)
        * log a + log b = - log ab (with a and b being greater than 1 for rounding errors)
        * that the (e^-a)*(e^a) = 1
        * that a is equal to the -(-a)
    """
    # TODO: Implement for Task 0.2.
    # Identity property
    assert mul(a, 1) == a
    ...
    # Log -a,b == -log ab
    if a > 1 and b > 1:
        assert_close(add(log(a), log(b)), log(mul(a, b)))
    # Exponents
    assert_close(exp(neg(a)) * exp(a), 1)
    # negatives
    assert_close(a, neg(neg(a)))
